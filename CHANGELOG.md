...
- **速率限制**：單 IP 12 次/分；**新增單 IP 100 次/日**（分鐘級擋不住「整天慢慢問」的耗帳單）。
- **全站斷路器（Denial-of-Wallet）**：跨所有 IP 每分鐘 60 次、每日 1500 次 LLM 上限（`globalLlmGate`），超過就不打 OpenRouter、改引導 LINE，保護帳單免被分散式攻擊灌爆。
- **呼叫 AI 前的廉價預過濾**（省 token）：
  - **注入／越獄偵測**：高信心攻擊字樣（`ignore previous instruction`、`system prompt`、`開發者模式`、「給我你的提示詞」等）→ 直接罐頭婉拒，**完全不呼叫付費 AI**。一般離題仍交給模型護欄（已驗證會婉拒）。
  - **重複洪水節流**：同 IP 60 秒內連送同一句，第 3 次起直接擋，不打 AI。
- **輸入瘦身**：每則訊息長度上限 4000 → **800 字**；單次請求 messages 上限 40 則（擋超大 body DoS）。
- **留資防灌爆**：`submit_inquiry` 單 IP 3 次/10 分、全站 100 次/日，超量不寫 DB／不通知老闆，改引導 LINE。
- 全站每日上限可用環境變數調整；提示詞層護欄（只答潛水、抗注入）自 v762 已在。
- 動檔：`src/app/api/assistant/route.ts`、`src/lib/rate-limit.ts`（匯出 `getClientIp` 共用取 IP 邏輯）。

## 20260701_771 - 2026-07-01 (AI 小幫手：改「導引式選單」三層漏斗，省 token + 手機友善)

- **版本尾碼**：自本版起移除歷史上無意義的 `M` 後綴，回到規範的 `YYYYMMDD_NN`（`770M` → `771`；流水號續增，排序不受影響）。

- **動機**：資料很固定，卻每題都丟給 AI（含很長的 system prompt + 即時資料 block）→ 浪費 token、慢、弱模型還常答錯日期。改成「先點選、答不到再問 AI、最後找真人」。
- **三層漏斗**：
  1. **L1 點選導引（主力，零 token / 零 DB）**：打開就給可點選單，階層下鑽到答案卡。內容取自 `assistant-kb.ts` 快照。
  2. **L1.5 即時查詢（零 token）**：場次／潛旅／裝備價目／取消政策／安全須知等會變動的資料，前端直接打既有公開 API（`/api/trips`、`/api/tours`、`/api/site-config`、`/api/config`，皆有版本號快取、命中零 DB），**不經過 AI**。
  3. **L2 打字問 AI**：選單答不到才打 `/api/assistant`（後端邏輯不變）。
  4. **L3 真人**：每一層永遠有「加 LINE 問汪汪教練」出口。
- **選單規模**：7 大類、6 分支、13 張靜態答案卡、7 個即時節點。常見問答（課程/費用/潛點/新手FAQ/預約）點選即答，不再耗 token、也不會被模型算錯日期。
- **新檔** `src/lib/assistant-menu.ts`：選單樹（前端純靜態常數，對應資料分層鐵則第 1 層）。
- **動檔** `src/components/assistant/ChatWidget.tsx`：新增快捷選單區（chips）、結構化答案卡、站內可點連結、即時查詢；打字問 AI 與 LINE 兜底維持不變。
- 相容性：後端 `/api/assistant`、招呼語/停用開關、mascot 樣式全部沿用，無破壞性變更。

## 20260701_770M - 2026-07-01 (AI 客服：修場次「星期」標錯一天 + 明天/週末補星期)

- v769 上線自測抓到：場次清單的「星期」少一天（如 7/4 標成「五」、應為「六」）。根因＝`runGetDiveSessions` 用 `T00:00:00+08:00` 再 `getDay()`，伺服器 UTC 下午夜+8 會退到前一天。改用 `weekdayOf(ds)`（中午 12:00+08 換算）即正確。
- 【即時資料】的「明天／本週末」也補上星期（`weekdayOf`），避免模型自己猜星期猜錯（曾把 7/2 猜成星期三、應為四）。
- 動檔：`src/app/api/assistant/route.ts`。

## 20260701_769M - 2026-07-01 (AI 客服：重設架構確保答案正確 + 多附可點選 URL)

- **問題**：弱模型（gemini-2.5-flash-lite）連簡單問題都答錯——要它「自己算日期 + 決定呼叫工具」太不可靠。
- **新架構（讀，不要算）**：每次請求後端先把**真實最新資料算好直接塞進 system prompt**，模型只要「讀」不用「算」：
  - 注入「今天日期、明天、本週末（已算好週六/週日確切日期）」。
  - 注入「近 30 天日潛場次」與「目前開放的潛旅團」（走版本號快取，命中零 DB，後台存檔即更新）。
  - 常見問題（今天/明天/本週末/有沒有團/名額）**完全不必呼叫工具**即可正確回答；工具只當「超過 30 天」的備援。
- **最高優先規則**：明令禁止自行推算日期或捏造場次/價格/名額，一律引用【即時資料】或後台價目政策。
- **多附可點選 URL**：回答盡量附 Markdown 連結讓客戶自己看詳情——場次表 `/schedule`、線上預約 `/pclogin`、線上詢問 `/contact`、課程 `/#courses`、潛點 `/#spots`、潛旅 `/#trips`、FAQ `/#faq`、LINE。
- **模型改 `google/gemini-2.5-flash`**（非 lite）：指令遵循/工具呼叫穩很多，仍便宜；後台或 `OPENROUTER_MODEL` 可覆寫。temperature 0.3→0.2。
...
- 「**存檔就更新知識**」達成方式：當日潛水(v765 `get_dive_sessions`)+ 潛旅(本版 `get_dive_tours`)都走**即時查詢 + 版本號快取**——寫入 `divingTrip`→bump `trips`、`tourPackage`→bump `tours`、`booking`→兩者皆 bump（`src/lib/prisma.ts`）。老闆在後台「日潛場次/潛水旅行」存檔，AI 下次查即最新，全自動、零手動。
- 知識庫規則：潛旅團況務必用 `get_dive_tours` 查；靜態潛旅清單僅為地點介紹。
- 動檔：`src/app/api/assistant/route.ts`、`src/lib/assistant-kb.ts`。

## 20260701_765M - 2026-07-01 (AI 客服：接「即時場次查詢」工具，能答真實場次/空位)

- AI 客服新增 `get_dive_sessions` 工具：被問「某天/這週/週末有沒有場次、有沒有位子」時，會去查**真實的近期日潛場次**（日期、星期、潛點、時間、岸潛/船潛、剩餘名額），照實回答，不再只說「請加 LINE」。
- 重用 `/api/trips` 同一套版本號快取（`cached(..., "trips", TTL_LISTING)`，命中零 DB；有人下單/取消自動失效）。
- 知識庫規則改：場次/空位**務必用工具查**再答；個人化報價仍不捏造、引導加 LINE；報名仍走 LINE。
- 動檔：`src/app/api/assistant/route.ts`、`src/lib/assistant-kb.ts`。
...
- 條件：訂單狀態 ∈ {未到場 no_show、客戶取消、天氣取消} 且 `現金(=paidAmount−creditUsed) > 0` 且 付款狀態非 refunding/refunded。折抵的抵用金已由系統自動退、不列入。
- chip 顯示待退筆數；有待退款時以紅色強調。啟用時略過其他篩選、直接列出全部待退款單（含過去場次）。純前端衍生，不改 schema、不自動扣款（客戶若選改期則自然不在現金未退之列）。
- 動檔 `src/app/admin/bookings/page.tsx`。
...
- 純前端按 `createdAt` 合併排序，不改後端；付款明細列保留 ✕ 刪除（未鎖定時），狀態列保留 actor/備註；無獨立時戳的「抵用金折抵/先前已付」用訂單成立時點排入。左欄狀態歷史 box 移除。
- 「訂單狀態下拉」「金額調整 / 帳務調整」等**控制項維持原處不動**（只合併兩個唯讀紀錄，不動操作區）。
- 動檔 `src/app/admin/bookings/page.tsx`。

## 20260630_753M - 2026-06-30 (訂單詳情：一鍵「現場收現・結清剩餘」)

- 老闆收到客戶**現場付的現金**時，💰 金額區新增一鍵鈕「💵 現場收現・結清剩餘 NT$ {剩餘款}」：點下去跳確認後，自動寫一筆「現金（實收）= 剩餘款」付款紀錄，剩餘款歸零、標記已付清。把原本「展開金額調整→選現金→補齊剩餘→確認」4 步併成 1 步。
- 沿用既有 `payment-entry` API 與防呆（總額未存先擋、剩餘≤0 不可結清），不另開後端；複用 `addingEntry` 忙碌狀態。原本逐筆／分期或其他付款方式仍走「➕ 金額調整」。
- 動檔 `src/app/admin/bookings/page.tsx`（新增 `settleCashOwed()`）。

## 20260630_752M - 2026-06-30 (訂單詳情：移除「修改總金額」+ 付款紀錄補日期)

- 訂單詳情/編輯視窗 💰 金額區移除「✎ 修改總金額」直接改總額的入口；金額變更**統一改走下方「🧮 帳務調整（加收/減免）」**，老闆用加收/減免項目調整應付與剩餘款，並寫入審計，避免直接覆蓋總額而無紀錄。原處改為提示文字導向帳務調整。
- 🧾 付款紀錄的「抵用金採用」「先前已付」兩列原本日期欄放的是標籤、沒有日期 → 改為顯示**訂單成立日**（折抵/收款發生時點），與一般明細列日期欄一致；標籤移到金額後方。
- 動檔 `src/app/admin/bookings/page.tsx`（順手移除已無用的 `editTotal` state）。

## 20260630_751M - 2026-06-30 (移除 Fun Dive 課程卡 — 與一日潛水重複)

- 「持證精進·Fun Dive 練功」本質就是持證者的**一日潛水**，不是真正的課程，與「一日潛水」預約／「日潛 Fun Dive」費用頁重複。從共用的 `COURSES` 移除該卡 → 全站（手機/桌機首頁課程區、LIFF 首頁、/course、/pricing、課程詢問）同步只剩 **體驗潛水 / OW / AOW** 三張。
- 不動 Fun Dive 概念本身：/pricing 的「日潛 Fun Dive（持證）」每支氣瓶費用、guide 頁、SEO 文案、一日潛水預約皆保留。無索引/slug 依賴，移除安全。動檔 `src/app/_home/data.tsx`。

## 20260630_750M - 2026-06-30 (課程詢問 — 併進正式預約頁第 4 分頁)

- 確認後把「課程詢問」併進正式潛水預約頁 `/liff/booking`：頂部分頁由 3 → **4**（一日潛水 / 旅行潛水 / 預約潛水 / **課程詢問**）。沿用既有 lazy 掛載 + 即時切換不重載。動檔 `src/app/liff/booking/page.tsx`。
- 公開預覽頁 `/course-inquiry` 保留：它**不需 LINE 好友**即可開（LIFF 分頁有好友 gate），可當網站端公開入口／分享連結。
- 課程詢問內容元件 `CourseInquiryContent` 同時被兩處共用。
...
## 20260629_744M - 2026-06-29 (到場點名排序：未點最前、已到場最後)

- 「到場點名」每個場次內的參加者改為依點名狀態排序：**未點（confirmed）最前 → 未到場（no_show）中間 → 已到場（completed）最後**。相同狀態維持原順序（穩定排序）。點完「到場」後即時重排到底部，名單由上往下越點越短。動檔 `src/app/admin/attendance/page.tsx`。

## 20260629_743M - 2026-06-29 (訂單編輯右欄精簡：人數/付款方式/金額調整/抵用金顯示)

- 「訂單詳情/編輯」右欄四項調整：
  - **移除「參加人數」可編輯欄**：左欄「下單明細」已顯示 ×N 人，不再重複。
  - **付款紀錄「先前已付」拆出抵用金**：`paidAmount` 內含下單時 `creditUsed`，現在單獨列「⭐ 抵用金採用 {amount}」，剩餘未明細化的現金才標「先前已付（未明細化）」。
  - **「金額調整（新增一筆付款）」改預設收合**：點標題才展開（保留手動記款能力，但平時不佔版面）。
  - **「付款方式」只在客戶已上傳付款憑證時才顯示**（付款方式是客戶上傳憑證時才寫入；沒憑證/待付款時隱藏）。
- 動檔 `src/app/admin/bookings/page.tsx`。

## 20260629_742M - 2026-06-29 (訂單編輯視窗合併單一兩欄 + 簽名/憑證並排)

- 「訂單詳情/編輯」改為**單一兩欄 grid、每欄各自獨立堆疊**，消除左欄（下單明細短）下方的大片留白：
  - **左視覺欄**：下單明細 → 備註/客戶備註/活動提醒 → 訂單狀態歷史 → 客戶簽名｜付款憑證（並排）。
  - **右視覺欄**：金額/付款紀錄/帳務調整/付款方式/訂單狀態 → 退款處理。
  - 用 grid `col-start`/`row-start` 指定欄位，只把「下單明細」搬進左欄開頭，payment/退款/備註大塊不動（降風險）。
- **客戶簽名移出備註框、改卡片式**，與**付款憑證並排各佔左欄一半（≈ 全寬 1/4）**；付款憑證內層卡片改單欄堆疊（窄欄更好讀）。
- 動檔 `src/app/admin/bookings/page.tsx`。

## 20260629_741M - 2026-06-29 (訂單編輯視窗底部改左右兩欄 + 加寬)

- 「訂單詳情/編輯」視窗加寬 ~30%（`max-w` 920→1200px），底部原本垂直堆疊的兩大塊改**桌機並排各佔一半**：左欄＝備註/客戶備註/活動提醒/客戶簽名/訂單狀態歷史/付款憑證；右欄＝💰退款處理（+ 未付款提示）。手機維持單欄堆疊。用 grid `col-start`/`row-start` 指定欄位、DOM 順序不動（風險低、不搬大塊 JSX）。目的：縮短視窗高度、桌機盡量免捲動。動檔 `src/app/admin/bookings/page.tsx`。

## 20260629_740M - 2026-06-29 (退款處理依訂單狀態 gating)

- 「訂單詳情/編輯」視窗的 **💰 退款處理** 改為雙條件啟動:**已付>0** 且 **訂單狀態進入「紅色區域」**(活動結束 / 活動取消:未到場・客戶取消・天氣・訂單不成立)才可用。其餘狀態(建立訂單/待確認匯款/已確認付款訂金/已完成付款)維持灰色、🔒、收合箭頭隱藏並附提示;點選跳 alert「目前訂單狀態不支援退款…請先改訂單狀態」。判斷用編輯視窗當下選的狀態(`deriveBookingDisplay`),改下拉即時連動、不需先存檔。純前端 UX 防呆,退款 API 未改。動檔 `src/app/admin/bookings/page.tsx`。

## 20260629_739 - 2026-06-29 (訂單編輯視窗縮窄兩欄 + 累計排除取消訂單)

- **累計只算有實際消費**:會員管理「潛水紀錄」的 訂單數/總人次/已付款,以及會員列表「累計消費(revenue)」、客戶詳情累積消費,**一律排除取消類(cancelled_*)與未到場(no_show)**——用戶取消無實際消費不應計入。三處同步:`admin/users/page.tsx`(潛水紀錄對話框)、`api/admin/users`(revenue)、`api/admin/customers/[userId]`(bookingCount/totalPaid)。取消訂單仍列在紀錄清單(只是不計入上方累計)。

- 「訂單詳情/編輯」視窗:寬度由 `min(95vw,1200px)` 縮為 `min(96vw,920px)`(原本太寬、單欄被拉得很開);桌機(lg)改**左右兩欄**:左=📋下單明細、右=金額/付款紀錄/新增付款/付款方式/訂單狀態/🧮帳務調整。退款處理、客戶備註、簽名、狀態歷史、按鈕、危險區維持全寬在下。動檔 `src/app/admin/bookings/page.tsx`。

## 20260629_738 - 2026-06-29 (訂單編輯加下單明細 + 老闆帳務調整)

- 桌機「訂單詳情/編輯」(`/admin/bookings` 編輯視窗)新增兩塊,解決「看不到客戶下單內容、沒分類」:
  - **📋 下單明細**:接 `PriceBreakdown` 元件,顯示客戶下單組成(氣瓶/船潛×支數×人數、裝備逐項、優惠、訂單總額 − 抵用金 = 應付)。
  - **🧮 老闆帳務調整**:手動加收費項目(項目名稱 + 金額,可正可負,例 共乘 +300、補上次沒潛水 −600)。加入即算進訂單總額/應付、列出可刪,寫入審計。新增 `POST/DELETE /api/admin/bookings/[id]/adjustment`,調整項存於 `priceBreakdown.bossAdjustments`。
  - `PriceBreakdown` 元件擴充:渲染 bossAdjustments(藍色列)並用調整後總額算應付。
- **訂單管理表格金額拆三欄**:原「金額 / 已付」改為 **總金額 / 抵扣金額 / 實際付款**(實際付款 = 已付 − 抵用金,即真正收到的現金;抵扣金額 = 下單折抵的抵用金)。空狀態 colSpan 同步 +1。
- 註:本版採加法插入(維持現有付款紀錄/新增付款/退款/狀態歷史/客戶備註);編輯視窗視覺左右兩欄待後續。
...
- 點任一筆訂單 → **底部彈窗(bottom sheet)** 滑出該筆完整詳細：客戶+電話(可直撥)、場次、**金額明細(總額/折抵/已付/應付)**、付款證明(查看匯款 + 確認收款/退回)。等於把該客戶「我的預約」攤開在老闆端。
- 詳細內容抽成共用元件 `src/components/admin-web/OrderDetail.tsx`；獨立詳細頁 `/admin/m/bookings/[id]`(新)與彈窗共用同一份，新增 `GET /api/admin/m/bookings/[id]`。
- 「訂單管理」(`/admin/m/bookings`)清單每筆改為可點 → 進訂單詳細頁。

## 20260629_733 - 2026-06-29 (手機訂單管理清單金額改顯示應付)

- 補 732 漏網:手機後台「訂單管理」(`/admin/m/bookings`)清單每筆金額走專屬 API `/api/admin/m/bookings`,原本回 `amount = totalAmount`(原始總額),有折抵用金的訂單顯示 575 而非 475。改為與「我的預約」一致:待付時 `amount = totalAmount − paidAmount`(已含抵用金折抵)、已付清/退款顯示總額。API select 補 `paidAmount`。
- 桌機「訂單管理」是表格(總額/已付分欄),語意清楚,維持不動;會員訂單歷史、合約費用、付款頁「總額」欄屬總額語意,亦不動。

## 20260629_732 - 2026-06-29 (全流程修「待匯款/應付」未扣抵用金)

問題:有折抵用金的訂單,多處「待匯款 / 應付金額」顯示原始總額(例 575)而非扣抵後應付(例 575−100=475)。全流程稽核並修正:

- **下單確認通知**(`booking_confirm`,LINE Flex + 站內 + Email):原本顯示「應付金額 = 原始總額」。改為傳 `creditUsed` / `payable(=總額−已付)`,有折抵用金時顯示「訂單總額 − 折抵用金 = 應付金額」三行,否則顯示應付。動檔 `api/bookings/daily/route.ts`、`lib/message-content.ts`、`lib/flex/booking-confirm.ts`。
- **付款提醒 cron**(D+7 最後通知 / D+2 友善提醒):金額由原始總額改為應付(總額−已付)。動檔 `api/cron/payment-reminders/route.ts`。
- **老闆結帳「已下單·待匯款」**(手機 `/admin/m/tonight` + 桌機 `/admin/tonight`):金額由原始總額改為應付,並標「已折 NT$X」。桌機 awaiting_verify 清單同步修正。
- 註:抵用金在「下單時」即折抵並記入 `paidAmount`,故應付一律 = `totalAmount − paidAmount`。客戶端「我的預約」「付款頁」於先前版本已正確。

## 20260629_731 - 2026-06-29 (手機後台改清單三區 + 老闆結帳聯合待辦)

- `/admin/m` 首頁從 2 欄卡片改為個人中心風格的「分區橫列清單」,分三區:**待我處理**(老闆結帳・客服信箱)、**今日現場**(日潛場次)、**查詢/管理**(會員管理・潛水旅行・抵用金管理)。訪客統計卡保留最上面。
- 移除「到場點名」卡(已在個人中心第一層);「訂單管理」「願望單」併入老闆結帳。
- 老闆結帳(`/admin/m/tonight`)改為**聯合待辦中心**:除原本 待匯款確認 + 已下單待匯款,新增「**新願望·待回覆**」段,並加「看全部訂單 / 看全部願望」常駐入口。首頁老闆結帳紅點 = 待確認匯款 + 待匯款 + 新願望加總。
- 修 `adminFetch` 的 abort 判斷改用 `e.name === "AbortError"`(不限 DOMException 實例),修正 LINE WebView 內偶發「signal is aborted without reason」紅字外洩;首頁有快取資料時不再跳載入失敗紅字。

## 20260629_730 - 2026-06-29 (我的預約收合列金額一致化)

- 「我的預約」收合列右側金額(`my/page.tsx:621`)原本顯示**訂單總額**(例 575),與展開後「應付金額」(例 475，已折 100 抵用金)不一致,看起來像打架。改為與下方同一套邏輯:待付時顯示**應付**(總額 − 已付含抵用金)、已付清時顯示總額。兩處數字一致。

## 20260629_729 - 2026-06-29 (個人中心身分徽章 + 後台管理連結)

- LIFF 個人中心(`/liff/profile`)標題區新增**身分徽章**:依 roles 顯示 IT / 老闆 / 管理 / 教練 / 助教(多重身分取最高;一般會員不顯示)。
- 「管理」區塊:`it/boss/admin` 新增「**後台管理**」連結 → `/admin/m`(手機後台,遵守 LIFF 不連桌機 /admin 的鐵則);`coach/assistant` 維持只有「教練到場點名」。
- 資料來源 `/api/me` 的 `roles`(已含 [role] backfill)。動檔 `src/app/liff/profile/page.tsx`。
