...
```dotenv
# ─── LINE ───
# ⚠️ 實際金鑰請勿寫進本檔（版控）！到 LINE Developers Console 取得後只填進 Zeabur 環境變數。
#    舊值已於 2026-05-11 外洩並曾提交於此，務必依 docs/runbooks/SECRET_ROTATION.md rotate。
LINE_CHANNEL_ACCESS_TOKEN=<在 Zeabur 設定，勿寫進版控>
LINE_CHANNEL_SECRET=<在 Zeabur 設定，勿寫進版控>
LINE_LIFF_ID=2010006458-fyokMnVv
NEXT_PUBLIC_LIFF_ID=2010006458-fyokMnVv

# ─── JWT / Cron ───（用 openssl rand -hex 32 產生，只填進 Zeabur，勿寫進版控）
JWT_SECRET=<在 Zeabur 設定，勿寫進版控>
CRON_SECRET=<在 Zeabur 設定，勿寫進版控>  # 需與 Cronicle 的 HAIWANGZI_CRON_SECRET 同值
...
| 名稱 | Cron 表達式 | URL |
| --- | --- | --- |
| 早提醒 | `0 1 * * *` (UTC 01:00 = 台北 09:00) | `https://haiwangzi.xyz/api/cron/reminders?token=<CRON_SECRET>` |
| 晚提醒 | `0 10 * * *` (UTC 10:00 = 台北 18:00) | `https://haiwangzi.xyz/api/cron/reminders?token=<CRON_SECRET>` |
...
```bash
curl -X POST "https://haiwangzi.xyz/api/admin/seed?secret=<LINE_CHANNEL_SECRET>"
```
