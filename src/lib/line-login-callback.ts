...
// v481：LINE Login callback 共用處理邏輯（v812 抽出成 lib）。
//   兩個 route 都指向它，避免「LINE Console 登記的 callback 路徑」與程式對不齊時整組失效：
//     · /api/auth/callback/line （目前 LINE Console 白名單；callbackUrl() 預設送這個）
//     · /api/auth/line/callback （舊路徑，保留相容——LINE Console 若改回這個也不會壞）
//   驗 state（防 CSRF）→ 換 token → 驗 id_token（audience=Login channel）
//   → 簽會員 web JWT 放 httpOnly cookie → 導回 next（預設 /pclogin）。
export async function handleLineLoginCallback(req: NextRequest): Promise<NextResponse> {
