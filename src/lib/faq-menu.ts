// v1074：點選式 FAQ 選單樹（前端純靜態常數，零 DB）。
//   前身是 v771 的 assistant-menu.ts；v1072 拔掉整組 AI 客服時一併刪除，
//   這裡只把「不經過 AI」的選單那一半救回來 —— 本檔與使用端都不碰任何 API key。
//   對應資料分層鐵則第 1 層：全域純靜態行銷內容（課程/潛點/潛旅地點/新手 FAQ/預約），
//   ⚠️ 會變動的即時資料（場次空位/潛旅團況/後台價目/政策）不要寫死在這裡——
//      標 `live`，面板直接打既有公開 API（皆有版本號快取，命中零 DB）。
...
export const FAQ_MENU: MenuNode[] = [
  {
    id: "courses",
    label: "🤿 我想學潛水（課程 / 費用）",
