...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const bookings = await prisma.booking.findMany({
      ...
      include: { user: { select: { displayName: true, realName: true, phone: true, email: true, lineUserId: true, notes: true } } },
    });
    ...
    const [trips, tours] = await Promise.all([
      dailyIds.length === 0 ? Promise.resolve([]) : prisma.divingTrip.findMany({
        ...
        select: { id: true, date: true, startTime: true, diveSiteIds: true, tankCount: true, pricing: true, isBoat: true },
      }),
        ...
        if (b.type === "daily") {
          ...
          if (t) {
            ref = {
              ...
              // v716：舊訂單明細估算用
              extraTank: ((t.pricing ?? {}) as { extraTank?: number }).extraTank ?? 0,
              baseTrip: ((t.pricing ?? {}) as { baseTrip?: number }).baseTrip ?? 0,
              isBoat: t.isBoat ?? false,
            };
          }
        } else {
