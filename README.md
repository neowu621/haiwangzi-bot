...
**當前版本**: `20260701_772`（見 `src/lib/version.ts`，每次 push 必 bump）
...
12. [安全機制](#安全機制)
13. [載入韌性機制（LINE WebView 三層防護）](#載入韌性機制line-webview-三層防護)
14. [品牌 / 設計系統](#品牌--設計系統)
15. [開發守則](#開發守則)
16. [疑難排解](#疑難排解)
...
身分拆兩個維度：**角色（基本帳號）** + **身分（職位，可多選疊加）**。`User.roles[]` 多重身分陣列；權限取聯集。

- **🪪 角色**：`customer` 會員 / `boss` 老闆（擁有者，最高）
- **🛠 身分（可疊加）**：`assistant` 助教 / `coach` 教練 / `admin` 管理者（老闆代理）/ `it` IT（技術全權）

| 功能 | 會員 | 助教 | 教練 | 管理者 | IT | 老闆 |
|---|:--:|:--:|:--:|:--:|:--:|:--:|
| 自己的預約/付款/抵用金 | 🔸 | 🔸 | 🔸 | 🔸 | 🔸 | 🔸 |
| 看場次 / 出團名單 / 勾到場（點名） | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 取消/改場次・天氣取消 | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ |
...
🔸 = 僅限自己的資料。**階層**：老闆 ≥ admin；**IT = 技術全權**（比照老闆，方便測試）。

**前台 / 後台**：會員 / 助教 / 教練 只走前台（LIFF；教練區看場次/點名，金流頁僅管理者+）；管理者 / IT / 老闆 才能登入後台（密碼）。

**指派規則**：助教/教練/管理者 → 後台「會員管理」可勾選（🪪 角色 / 🛠 身分 兩區塊，多選）。**老闆 / IT 高權角色只能由系統腳本（資料庫 / `bootstrap-roles` 端點）設定**，介面不可新增/移除。

**訊息通知對象**：
- 👤 客戶（notifyCustomer）：預約確認、訂金/尾款、行前、天氣取消、付款退回、抵用金、退款…
- 🛠 老闆/管理者（notifyAdmins）：新付款待核、新退款申請、新願望單
- 🤿 教練（notifyCoaches）：超賣警示

> 詳細權限對照另見 `README.html`（可直接瀏覽器開的視覺版）。
...
## 載入韌性機制（LINE WebView 三層防護）

> **背景（為什麼需要）**：LINE 內建 WebView 不會真正關閉頁面，只會「凍結」放背景（可能數小時～數天）。
> 期間若我們部署了新版，舊頁面引用的 Next.js JS chunk（hashed 檔名）已從伺服器消失 →
> 使用者回到 LINE 喚醒舊頁、切頁時 chunk 404 → 畫面卡在載入動畫 10 秒以上，
> 過去唯一解法是「關閉 LINE App 重開」。以下三層防護把這件事自動化／可自救。

| 層 | 版本 | 機制 | 程式位置 |
|---|---|---|---|
| ① 預防 | v1003 | 離開 App **超過 5 分鐘**後回到前景（`visibilitychange`/`pageshow`）→ 打輕量 `/api/healthz` 比對版本；與本頁 build 版本不一致 → 自動 `location.reload()` 換新 bundle（等同自動「關 App 重開」）。另監聽 `ChunkLoadError` 類錯誤 → 自動 reload 一次（60 秒防迴圈）。 | `src/lib/liff/LiffProvider.tsx` |
| ② 自救 | v465 | 所有 API 請求 `cache: "no-store"` + **12 秒逾時**自動中止；**唯讀請求**（無 body、無外部 signal）逾時／連線層錯誤時，用**全新連線**自動重試一次（25 秒逾時）。⚠️ POST（下單/付款）**不自動重試**——防止重複下單，只回明確錯誤讓使用者手動重試。 | `src/lib/liff/LiffProvider.tsx`（fetchWithAuth）＋ `src/lib/admin-web-auth.ts`（adminFetch，後台同款） |
| ③ 引導 | v1004 | 載入動畫連續顯示**超過 8 秒** → 顯示「🔄 重新載入」一鍵按鈕 ＋「若仍無法使用請關閉 LINE 重開」備案文字。共用元件一改全站生效（12+ 個載入畫面）。 | `src/components/shell/LiffLoading.tsx` |

**設計取捨備忘**：
- ①的「5 分鐘」門檻：避免使用者填表單中途切去 LINE 聊天室、切回來剛好遇到部署被 reload 洗掉表單。長時間凍結（真正會壞的情境）必然超過 5 分鐘。
- ②的重試只限唯讀：GET 重試無副作用；POST 重試可能重複下單/重複扣抵用金，寧可讓使用者看到錯誤自行重按。
- ③的「8 秒」：3–5 秒會誤傷正常的 4G/冷啟動載入；10 秒使用者早已不耐。8 秒是「明顯異常」分界。
- 版本比對來源：`/api/healthz` 回傳 `version`（`src/lib/version.ts` 的 `APP_VERSION`，每次部署 bump）。
